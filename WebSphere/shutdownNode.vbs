...
' shutdownNode
'
' PURPOSE: safely shutdown given WebSphere server by stopping all WAS Application servers and node agents on that machine
' 1) Read configuration file, 2) analyze installed profiles, 3) per profile: detect nodes, 
' LAST EDITED: 10-05-27
' AUTHOR(S): Berndtgen, Düsseldorf, 2010
' SYNTAX: cscript shutdownNode.vbs -s[erver] <server> [[-]?]
' VERSION: 1.1
'
...
' " -port 7101 -user user_id -password #####"
...
sub Init
	...
	printf "Now going to shutdown all WebSphere processes on " & sServerLongname & " (" & sServersIPAddress & "), using Account " & sUserInfo & " with Port " & sDMGRPort & " in " & sEnvironment & " environment." & vbCRLF
	on error goto 0
...
sub GetNodenames(ByVal sWasAdmin, ByVal sACGetNodenames)
	...
	if bVerbose then printf "GetNodenames: executing " & sWasAdmin & " " & sUserInfo & " -port " & sDMGRPort & " -c " & sACGetNodenames & vbCRLF
...
'
Sub ShowUsage()
    Wscript.echo "shutdownNode" & _
    "Safely shutdown given WebSphere server by stopping all WAS Application servers and node agents on that machine" & vbCRLF & vbCRLF & _
    "cscript shutdownNode.vbs -s[erver] [-l[ogfile] <logfilename>] [-v[erbose]] [[-]?]" & vbCRLF & _
    ...
    "example: cscript shutdown.vbs -s server -p7101 -u "" -user user_id -password ####"" -v -c .\shutdownNode.conf"
End Sub
